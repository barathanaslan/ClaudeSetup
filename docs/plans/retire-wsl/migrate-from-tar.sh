...
H=/c/Users/Barat
P=home/barathanaslan                   # prefix inside the tar
...
"$T" -xf "$TAR" -C "C:/Users/Barat" --strip-components=2 "${X[@]}" \
...
mkdir -p /e/ML
"$T" -xf "$TAR" -C "E:/ML" --strip-components=3 "${X[@]}" --exclude="$P/ml/bin" \
  "$P/ml" 2>&1 | grep -v "symlink\|Removing leading" | tee -a "$LOG"
log "   exit=${PIPESTATUS[0]}"

log "3/6 huggingface cache -> E:/ML/hf-cache"
mkdir -p /e/ML/hf-cache
"$T" -xf "$TAR" -C "E:/ML/hf-cache" --strip-components=4 "$P/.cache/huggingface" 2>&1 | grep -v "symlink\|Removing leading" | tee -a "$LOG"
log "   exit=${PIPESTATUS[0]}"

log "4/6 dotfiles -> staging $H/.wsl-home  (merged by migrate-dotfiles.py, nothing overwritten blindly)"
mkdir -p "$H/.wsl-home"
"$T" -xf "$TAR" -C "C:/Users/Barat/.wsl-home" --strip-components=2 \
