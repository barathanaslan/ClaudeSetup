...
Agent tool call:
  ...
  model: opus
  prompt: "Execute the plan at docs/plans/plan-NNN-<name>.md. Read docs/ for project context first. The plan is self-contained: follow its steps, respect its constraints, and verify every acceptance criterion before reporting done."
```
...
Agent tool call:
  ...
  run_in_background: true
  model: opus
  prompt: "Review the implementation of docs/plans/plan-NNN-<name>.md: read the diff, re-run whatever verifies the acceptance criteria, and report pass/fail per criterion with evidence, plus anything the implementation broke."
```
...
docs/
  ...
  ledger.md      # Campaigns only: append-only experiment log. Created on the first experiment.
...
- **ledger.md** — campaigns only (see Campaigns): one row per experiment, append-only, never edited after the fact. The one docs file where history accumulates on purpose.
...
- **Ledger**: appended by whoever ran the experiment (worker agents included), immediately after the harness reports — never batched up or reconstructed later.
